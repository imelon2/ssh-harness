...
function truncateAtCodepointBoundary(buf, maxBytes) {
    if (maxBytes === 0) {
...
export function runSsh(host, argv, opts) {
    const sshArgv = [
        '-F', opts.configPath,
    ...
    const stdoutResult = truncateAtCodepointBoundary(result.stdout, opts.maxStdoutBytes);
    const stderrResult = truncateAtCodepointBoundary(result.stderr, opts.maxStderrBytes);
    const timedOut = (result.signal === 'SIGTERM' && durationMs >= opts.timeoutMs - 50) ||
