...
import { buildRegistry, expandWildcardHosts, lintAllowlist } from './allowlist.js';
import { buildShape } from './schema.js';
import { renderArgv } from './template.js';
import { runSsh } from './exec.js';
import { appendAudit, redactParams, AuditAppendError } from './audit.js';
import { loadHostAliases } from './ssh_config.js';
import { loadConfig, resolveSshConfigPath } from './config.js';
...
export function createServer(env = process.env) {
    ...
    const config = {
        ...baseConfig,
        sshConfigPath: resolveSshConfigPath(baseConfig.sshConfigEnvOverride, baseConfig.allowlistPath, registry.sshConfigRoot()),
    };
    ...
    try {
        ...
        console.error(`[ssh-harness] could not read ssh_config at ${config.sshConfigPath}: ${err.message}`);
    ...
    const server = new McpServer({ name: 'ssh-harness', version: VERSION }, { capabilities: { tools: {} } });
    for (const tool of tools) {
        server.registerTool(tool.name, { description: tool.description, inputSchema: tool.paramsSchema.shape }, async (args) => tool.handler(args));
    }
 ...
 */
export async function executeRuleCall(registry, ruleId, rawArgs, config) {
    ...
    if (!parsed.success) {
        const ev = baseEvent(rule.id, rule.tool.name, 'schema_error', redactParams(rawArgs, rule.params), [], '', null, 0, '', '', parsed.error.message);
        safeAppend(config, ev);
    ...
    if ('host' in args) {
        ...
        if (!registry.hosts().includes(h)) {
            const ev = baseEvent(rule.id, rule.tool.name, 'host_denied', redactParams(args, rule.params), [], h, null, 0, '', '', `host '${h}' not in allowlist hosts`);
            safeAppend(config, ev);
    ...
    try {
        ...
        const ev = baseEvent(rule.id, rule.tool.name, 'schema_error', redactParams(args, rule.params), [], '', null, 0, '', '', err.message);
        safeAppend(config, ev);
    ...
    try {
        ...
        const ev = baseEvent(rule.id, rule.tool.name, 'exec_error', redactParams(args, rule.params), redactArgv(argv, rule.params, args), host, null, 0, '', '', err.message);
        safeAppend(config, ev);
        return errorResult(`exec error: ${err.message}`);
    }
    // e. Build audit event
    ...
    try {
        ...
        if (err instanceof AuditAppendError) {
            ...
            try {
                const failEvent = { ...event, outcome: 'audit_failed', errorMessage: err.message };
...
function baseEvent(ruleId, toolName, outcome, params, argv, host, exitCode, durationMs, stdoutTail, stderrTail, errorMessage) {
    return {
        ts: new Date().toISOString(),
...
function safeAppend(config, event) {
    try {
