...
import { appendAudit, AuditAppendError, } from './audit.js';
...
export function buildBuiltinTools(registry, config) {
    const tool = {
        ...
        handler: async (args) => executeBuiltinCall(registry, config, args),
    };
    return [tool];
}
export async function executeBuiltinCall(registry, config, rawArgs) {
    ...
    const event = makeEvent(unreachableCount === 0 ? 'ok' : 'exec_error', { checkHealth: true }, ['__builtin__', 'check'], '', unreachableCount === 0 ? 0 : 1, totalDuration, `${reachableCount}/${results.length} reachable`, truncate(firstError, HEALTH_CHECK_ERROR_TRUNCATE));
    try {
        appendAudit(config.auditPath, event, { bestEffort: config.auditBestEffort });
    }
    catch (err) {
...
// --- helpers ---
function makeEvent(outcome, params, argv, host, exitCode, durationMs, stdoutTail, stderrTail, errorMessage) {
    return {
...
function secondaryAudit(config, event, message) {
    try {
        const failEvent = { ...event, outcome: 'audit_failed', errorMessage: message };
...
function safeAudit(config, event) {
    try {
