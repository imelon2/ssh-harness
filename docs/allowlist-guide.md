...
| `maxStdoutBytes` | 262144 (256 KiB) | stdout 캡. 초과분은 UTF-8 codepoint 경계에서 잘리고 `truncated:true` |
| `maxStderrBytes` | 65536 (64 KiB) | stderr 캡 |
...
**Pattern 작성 시 강제 규칙:**
- **`-`로 시작하는 문자열을 매치하면 안 됨.** lint가 `^<pattern>`을 `-`와 `-aaa...`로 테스트하여 매치되면 reject (`src/allowlist.ts:299-318`).
  - `[a-zA-Z0-9]+` → OK (영숫자 강제)
  - `[\w-]+` → reject (dash로 시작 가능)
  - `.+` → reject
  - `^-?[a-z]+$` → reject (anchor 떼고 테스트되므로)
- **nested quantifier 회피.** `(a+)+`, `(a*)*` 같은 ReDoS 패턴은 `[WARN]` 발생 (`UNSAFE_PATTERN_RE`).
- anchor(`^`, `$`)는 자동으로 적용된다고 가정하지 마세요. Zod의 `.regex()`는 부분 매치를 허용합니다. **항상 `^...$`로 명시적으로 anchor 박으세요.**

```yaml
# 나쁜 예: 'foo-bar; rm -rf /' 도 부분매치로 통과
pattern: '[a-z]+'
